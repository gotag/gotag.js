/*
 * jQuery GoTag Upcoming Events Plugin 0.1
 *
*/
(function($){
	...
	var FormattableDate = function(date){
		...
		UpcomingEvents = function(el, opts){
			...
			getObjects: function(eventSlug){
				var url = this.options.apiUrl + "events.jsonp" + "?key=" + this.options.apiKey + "&callback=?",
			...
			getEventsList: function(){
				...
				$.when(this.getObjects()).then(function(data){
				  $.each(data.events, function(idx, eventObj) {
						self.events[eventObj.id] = eventObj;
					});
					self.totalEvents = 1;//data.total_count;
					self.showEventsList();
				});
			},
			  ...
			  var self = this,
				eventEl = $('<tr class="tl-upcoming-item tl-upcoming-event">' +
								'<td class="tl-upcoming-item-label tl-upcoming-event-label">' +
								  '<div class="tl-upcoming-event-flyer"><img src="' + eventObj.flyer.medium.url + '" width="60" /></div>' +
